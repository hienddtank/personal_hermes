---
name: mbox-parser
description: Parse .mbox email archive files, filter noise (Google/ads), group into threads via In-Reply-To/References, strip HTML from bodies, export to Excel + JSON. Handles large files (2.6GB+) with streaming line-by-line parsing and checkpoint resumption.
category: email
---

# MBOX Email Parser

Parses .mbox email archives in two phases:
1. **Phase 1:** Stream-parse the mbox file line-by-line → raw JSON with full body (HTML stripped)
2. **Phase 2:** Filter noise + group into threads via In-Reply-To/References → Excel + JSON with thread body
...
If a command times out during parsing, the script saves progress to `.parse_checkpoint.json` every 500 emails. Re-running resumes from where it left off instead of starting over. Clean up with:
```bash
rm .parse_checkpoint.json .phase1_emails.json
```

## Phase 1: Stream Parse (with checkpoint + HTML stripping)
...
1. **Parse:** `python parse_mbox.py` — Streams mbox, extracts full body (HTML stripped), saves checkpoint every 500 emails
2. **Re-run after timeout:** Script auto-resumes from `.parse_checkpoint.json`
3. **Clean start:** `rm .parse_checkpoint.json .phase1_emails.json`
