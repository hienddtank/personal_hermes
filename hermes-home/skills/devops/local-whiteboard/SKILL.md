...
# → {"ok": true, "entry": {"id": "gEjs4d0nd15WQ", "name": "file.pdf", "mime": "application/pdf", "size": 12345, "ts": ..., "url": "/file/gEjs4d0nd15WQ?key=meeeeeee"}}
```
...
# Full board JSON (text + images array + files array)
...
**CRITICAL:** Image paste type is `paste_image`, NOT `image_paste`. This is a common mistake.
...
async def push_text():
    async with websockets.connect("ws://127.0.0.1:8765/ws?key=meeeeeee") as ws:
        ...
        # Send text
        await ws.send(json.dumps({"type": "text_update", "text": "# Hello World\nThis is my whiteboard."}))
...
### Option A: ngrok tunnel (recommended for occasional access)

```bash
# Check if ngrok is already running a tunnel
...
# Start a new tunnel (if ngrok CLI available):
...
# Alternative with pyngrok SDK (from Python):
python3 -c "
from pyngrok import ngrok
tunnel = ngrok.connect(8765, 'http')
print(f'Public URL: {tunnel.public_url}')
print(f'Share link: {tunnel.public_url}/b/meeeeeee')
"

# Kill existing tunnel first if needed:
ngrok stop  # or: kill $(pgrep -f "ngrok http 8765") && ngrok http 8765 &
```

**Important for WebSocket access over ngrok:**
- Free tier tunnels idle out after ~60s — implement `ping`/`pong` heartbeats (already built into the whiteboard)
- Clients using WS must reconnect periodically (exponential backoff: 1s → 2s → 4s → max 30s)
- Use `wss://` URLs when the ngrok tunnel serves HTTPS, `ws://` for HTTP
...
### Push a file to the whiteboard:
1. Write file to container filesystem (or download from URL)
2. POST to `http://127.0.0.1:8765/upload?key=<key>` with the file
3. The file appears in the board for all connected clients

### Pull all board content:
1. GET `/download.json?key=<key>` for metadata
2. Download individual files via `/file/{id}?key=<key>`
3. Images served at `/img/{id}?key=<key>`

## Screenshot Capture (Headless Chrome)

To capture screenshots of the whiteboard UI from inside the container:

1. **Install dependencies:** `apt-get install -y chromium` and `cd /tmp && npm init -y && npm install puppeteer`
2. **Run Puppeteer with these flags** (required when running as root in containers):
   ```javascript
   ...
   ```
3. Navigate to `http://127.0.0.1:8765/b/<key>` and take screenshot
...
- **From outside LAN**: Use actual host IP or set up a tunnel (ngrok, etc.).
- **Board not found**: Data lives in `whiteboard_instances/<key>/`. New board auto-created on first access.
- **WebSocket text update silently ignored**: You're using `"type": "text"` — it must be `"type": "text_update"`. The server ignores unknown message types.
- **Chrome crashes as root**: Add `--no-sandbox --disable-dev-shm-usage` flags to puppeteer launch args.
