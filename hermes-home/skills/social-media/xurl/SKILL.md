...
The older `xitter` skill wrapped `x-cli` (https://github.com/Infatoshi/x-cli), a third-party Python CLI. `xurl` is preferred — it's maintained by the X developer platform team, supports OAuth 2.0 PKCE with auto-refresh, covers a larger API surface, and has better token safety.

If you already have `x-cli` installed and configured, the basic command patterns are:
...
Prefer `xurl` for new setups. Migrate existing x-cli users to xurl by following the One-Time User Setup section above.
